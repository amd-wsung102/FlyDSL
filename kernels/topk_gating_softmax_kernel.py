...
def _if_then(if_op):
    ...
    emit dynamic predicates with explicit `scf.IfOp` when called from a
    sub-function (the @flyc.kernel AST rewriter that auto-lowers Python
    `if` to scf.IfOp only fires at the top of the kernel source).
    """
...
def compute_topk_gating_layout(num_experts: int, topk: int, dtype_str: str):
    ...
    layout decisions used by both the standalone gating kernel and the fused
    moe_softmax_sort kernel.

    Centralised so the fused kernel can compute the same constants without
    duplicating the heuristics. Returns a dict of all derived constants;
    raises ValueError when the layout heuristic cannot find a suitable
    shape for the requested (num_experts, topk).
    """
...
def emit_topk_gating_softmax_body(
    ...
    *,
    ...
    # is present in both versions and produces the same IR.
...
def build_topk_gating_softmax_module(
    ...
    def topk_gating_softmax_kernel(
        ...
        # Body is shared with the fused MoE softmax+sort kernel. Keeping a
        # single implementation avoids divergence between the standalone
        # and fused pipelines as the gating math evolves.

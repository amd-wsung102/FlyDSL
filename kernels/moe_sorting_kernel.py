...
_decode_fused_cf_cache = {}  # fused decode: same key + (dtype_str, renormalize) -> CompiledFunction
...
# ---------------------------------------------------------------------------
# FlyDSL GPU kernel — fused (gating + sort) decode path
# ---------------------------------------------------------------------------
@functools.lru_cache(maxsize=256)
def compile_moe_sorting_decode_fused(
    *,
    ...
    """Compile the fused (gating + sort) decode-path MoE kernel.
    ...
    > 0 zero the moe_buf as in the unfused decode kernel.

...
_decode_fused_cf_cache = {}  # (num_experts, topk, max_tokens, unit_size, n_grid_blocks, dtype_str, renormalize, has_mask) -> CompiledFunction (V2: LDS-direct, no HBM scratch)
...
# ---------------------------------------------------------------------------
# FlyDSL GPU kernel — fused decode path (gating + softmax + topK fused into
# the same kernel as moe_sorting decode).
# ---------------------------------------------------------------------------
@functools.lru_cache(maxsize=256)
def compile_moe_sorting_decode_fused(
    *,
    num_experts: int,
    topk: int,
    dtype_str: str = "bf16",
    ...
    """Compile the fused (gating + sort) decode-path MoE kernel (V2:
    LDS-direct on-chip pipeline).

    The kernel reads `gating_logits[M, E]` directly, performs the gating
    softmax + top-K in block 0, stages all per-token-K intermediates in
    on-chip LDS, then runs the sort phases (count → prefix-sum → scatter +
    padding) without ever round-tripping the routing intermediates through
    HBM. Phase 1 of the original sort is gone — gating's leader-write
    callback fills the mesh directly. Phase 3's per-token weight load now
    reads from a small `weights_lds[max_tokens, topk]` region instead of
    HBM. Blocks > 0 zero the moe_buf as in the unfused decode kernel.

    Parameters identical to ``compile_moe_sorting_decode`` plus:
    ...
    r_for_sub = min(r_for_sub, r_token_min)
    sub_tokens = r_for_sub
    smem_rows = sub_tokens + cumsum_bufs
    ...
    # HBM. ≤512 B at the decode-path bound (max_tokens=16, topk=8) so it
    ...
    gating_layout = compute_topk_gating_layout(E, topk, dtype_str)
    ...
    @flyc.kernel
    def moe_sorting_decode_fused_kernel(
        ...
        with _if_then(_if_sort):
            ...
            # =========== PHASE 0 (V2): Gating + softmax + top-K ==============
            ...
            c_max_tokens = fx.Int32(max_tokens)
            ...
            emit_topk_gating_softmax_body(
                gating_logits,
                None,           # TopkWeights HBM not used in V2
                None,           # TopkIndices HBM not used in V2
                None,           # TokenExpertIndices HBM not used in V2
                i32_tokens,
            ...
            for i_e2 in range_constexpr(0, E, BLOCK_SIZE // 8):
                ...
                for i_sub2 in range_constexpr(0, sub_tokens, 8):
                    ...
                    # V2: weight comes from LDS (gating staged it there)
    ...
    @flyc.jit
    def launch_moe_sorting_decode_fused(
        gating_logits: fx.Tensor,
        sorted_token_ids: fx.Tensor,
        sorted_weights_out: fx.Tensor,
        sorted_expert_ids: fx.Tensor,
        ...
        launcher = moe_sorting_decode_fused_kernel(
            gating_logits,
            sorted_token_ids,
            sorted_weights_out,
            sorted_expert_ids,
    ...
    return launch_moe_sorting_decode_fused
...
_topk_fallback_scratch_cache = {}  # (device, num_tokens, topk, dtype) -> (topk_weights, topk_ids, tei)
_topk_fallback_builder_cache = {}  # (num_experts, topk, dtype_str, renormalize) -> launch_fn


def _supports_fused_decode(num_experts: int, topk: int, dtype_str: str) -> bool:
    """Whether the fused decode kernel supports this configuration.

    Matches `compute_topk_gating_layout`'s constraints: returns False when
    the gating layout heuristic cannot find a valid (VPT, THREADS_PER_TOKEN)
    pair for `num_experts`, or when `topk > num_experts`.
    """
    try:
        compute_topk_gating_layout(num_experts, topk, dtype_str)
        return True
    except ValueError:
        return False
...
def moe_softmax_sort_flydsl(
    ...
    For decode (M ≤ DECODE_MAX_T) with a supported (num_experts, topk, dtype)
    ...
    fusion_ok = (
        M <= min(sub_tokens, DECODE_MAX_T)
        and topk <= num_experts
        and _supports_fused_decode(num_experts, topk, dtype_str)
    )
    ...
    if fusion_ok:
        ...
        launch_moe_sorting_decode_fused_path(
            gating_logits,
            sorted_ids, sorted_weights, sorted_expert_ids, num_valid_ids,
            moe_buf_i32, mask_tensor, M, moe_buf_elems, n_grid_blocks,
            num_experts=num_experts, topk=topk,
...
def launch_moe_sorting_decode_fused_path(
    gating_logits,
    sorted_ids, sorted_weights, sorted_expert_ids, num_valid_ids,
    moe_buf_i32, expert_mask, i32_tokens, i32_moe_buf_elems, n_grid_blocks,
    *,
    ...
    """Low-level launcher for the fused (gating + sort) decode path.

    V2 LDS-direct: reads `gating_logits[M, E]` and emits the sort outputs
    ...
    cf = _decode_fused_cf_cache.get(cache_key)
    if cf is not None:
        stream = torch.cuda.current_stream()
        cf(gating_logits,
           sorted_ids, sorted_weights, sorted_expert_ids, num_valid_ids,
    ...
    launch_fn = compile_moe_sorting_decode_fused(
        num_experts=num_experts,
        topk=topk,
        dtype_str=dtype_str,
        renormalize=renormalize,
    ...
    _decode_fused_cf_cache[cache_key] = cf

...
def check_sorted_weights(ref_w, gpu_w, ref_ids, topk, M, atol=1e-5, label="sorted_weights",
                         gpu_ids=None, num_padded=None):
...
def run_test(T, E, topk, unit_size=UNIT_SIZE, max_tokens=None):
    ...
    ref_ids, ref_w, ref_eids, ref_nvalid = moe_sorting_reference(
        topk_ids, topk_weights, E, unit_size
    )
...
def run_test_vs_aiter(T, E, topk, unit_size=UNIT_SIZE, max_tokens=None):
    ...
    try:
        ...
        print(f"  [SKIP] aiter not available for cross-validation")
        return None, None
...
@pytest.mark.parametrize("T,E,topk,dtype_str", FUSED_DECODE_CONFIGS)
def test_moe_softmax_sort_fused_decode(T, E, topk, dtype_str):
    assert _supports_fused_decode(E, topk, dtype_str), (
        f"Test config {E=}/{topk=}/{dtype_str=} not supported by fused decode; "
        "check FUSED_DECODE_CONFIGS"
    )
    passed = _run_softmax_sort_fused_test(T, E, topk, dtype_str)
    assert passed, (
        f"moe_softmax_sort_flydsl fused decode mismatch for "
        f"T={T}, E={E}, topk={topk}, dtype={dtype_str}"
    )


@pytest.mark.parametrize("T,E,topk,dtype_str", [
    # M > DECODE_MAX_T forces the fused entry to take its 2-kernel fallback
    # (separate gating + moe_sorting launches). The gating layout must
    # still be supported; this is the only fallback shape currently
    # available because the standalone gating kernel shares the same
    # _pick_layout heuristic.
    (32,   256, 8, "bf16"),   # meshless regime
    (64,   256, 8, "bf16"),   # meshless regime
    (128,  256, 8, "bf16"),   # meshless regime
    (1024, 256, 8, "bf16"),   # prefill regime
])
def test_moe_softmax_sort_fallback(T, E, topk, dtype_str):
    """The fused-path entry must remain correct when it falls back to the
    2-kernel chain because M exceeds the decode bound."""
    passed = _run_softmax_sort_fused_test(T, E, topk, dtype_str)
    assert passed, (
...
def bench_eager_us(fn, warmup=BENCH_WARMUP, iters=BENCH_MEASURE, flush_l2=True):
    ...
    if flush_l2:
        ...
        flush_buf = torch.empty(max(l2_bytes * 2, 8 * 1024 * 1024),
...
def run_bench_comparison(token_sweep=None):
    ...
    for T in token_sweep:
        ...
        topk_ids = torch.stack(
            [torch.randperm(E, device="cuda")[:topk] for _ in range(T)]
        ).to(torch.int32)
...
def run_fused_bench_comparison(token_sweep=None, dtype_str="bf16",
    ...
    M values, in both eager and CUDA-graph modes. Saves the table as the
    V1 baseline for the V2 follow-up to compare against.
    ...
    if token_sweep is None:
        ...
        # DECODE_MAX_T=16 so the fallback path is also exercised.
...
def main():
    ...
    parser.add_argument("--bench", action="store_true",
                        help="Run benchmark sweep (eager + graph, FlyDSL vs CK)")
    parser.add_argument("--bench-fused", action="store_true",
                        help="Run fused-vs-unfused benchmark for moe_softmax_sort_flydsl "
                             "(captures the V1 baseline for the V2 follow-up)")
    parser.add_argument("--bench-tokens", type=str, default=None,
                        help="Comma-separated T values for bench (default: all)")
